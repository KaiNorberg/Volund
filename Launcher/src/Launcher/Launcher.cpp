...
void Launcher::Procedure(const VL::Event& e)
{
	...
	case VOLUND_EVENT_TYPE_RENDER:
	{
		...
		this->m_Renderer->Submit(this->m_GameState, this->m_Framebuffer);
		this->m_Renderer->End();

		this->m_Framebuffer->BlitToScreen();
	}
	break;
...
Launcher::Launcher()
{
	...
	this->m_GameState = VL::GameState::Create(this->GetDispatcher(), MAIN_SCENE);
}
